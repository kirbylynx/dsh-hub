...
M4D-7B plugin 安装/入伙/启动体验基线：这是 DSH 进程内插件交付方式的连接状态、session/workspace 诊断摘要、browser card live status bridge、显式远程 host capability overlay、一行启动包装 `dsh-hub-web`、只读安装检查 `plugin-install-check`、profile 安装器 `plugin-install` 和入伙 CLI `plugin-join` 基线；它保留 M4D-3 入伙/凭据 runtime、M4D-2 tunnel adapter、M4D-1 browser settings card 与 M4C 目录选择 overlay。
