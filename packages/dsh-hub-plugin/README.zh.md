...
- 提供 `PluginCredentialStore` 和 `PluginRuntime`：用 registry key 或 replacement grant 调用既有 `/api/register` 时上报 `delivery=plugin` 和可选的非秘密 `deploymentMode`，成功后仅保存 endpoint、installationId、instanceId、instanceToken、过期/续期时间、target、版本和 deployment mode 等 instance credentials；
...
- 声明 `dsh.client` 和 `./client` lazy-CJS browser bundle，在 DSH Plugins settings 页注册一个 `dsh-hub` 状态、诊断与 hosted 模型设置卡片；
- hosted 模式下提供同源 `/plugins/dsh-hub-plugin/model-settings.json` 与 `/plugins/dsh-hub-plugin/model-settings/test.json` 端点，以及面向 DeepSeek 官方和 OpenAI-compatible/custom Base URL provider 的窄面设置面板，支持 OpenAI Chat Completions 与 Responses API 协议选项；
...
hosted DSH composition 应显式设置非秘密 deployment mode：
...
- browser card 会同源 `fetch` `/plugins/dsh-hub-plugin/status.json` 读取最小脱敏 `statusView`；仅 hosted 模式下，它也可以调用同源模型设置端点。Hub service 不解析、不持久化 provider API key；key 只写入 hosted DSH 的本地 credential store；
